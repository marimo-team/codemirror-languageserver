...
describe("hover contents", () => {
    // BUG: hover contents from the server must be sanitized before they reach
    // innerHTML, so a hostile `<img src=x onerror=...>` never becomes a live
    // node — currently `marked` passes raw HTML through and the <img> is
    // created (its onerror handler is armed).
    it.fails(
        "does not create a live img element from hover contents",
        async () => {
...
describe("diagnostic messages", () => {
    // BUG: a diagnostic message is server-controlled text and must be
    // sanitized before being assigned to innerHTML — currently
    // `body.innerHTML = this.markdownRenderer(message)` creates a live <img>
    // with an armed onerror handler.
    it.fails(
        "does not create a live img element from a diagnostic message",
        async () => {
...
describe("signature help documentation", () => {
    // BUG: signature documentation from the server must be sanitized before it
    // reaches innerHTML — currently the hostile markup renders as a live <img>
    // inside .cm-signature-docs.
    it.fails(
        "does not create a live img element from signature documentation",
        async () => {
    ...
    // BUG: parameter documentation from the server must be sanitized before it
    // reaches innerHTML — currently the hostile markup renders as a live <img>
    // inside .cm-parameter-docs.
    it.fails(
        "does not create a live img element from parameter documentation",
        async () => {
            const dom = await renderSignatureHelp(
                {
                    label: "f(x)",
