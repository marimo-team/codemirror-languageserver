...
/** Attaches extra request stubs to the partial client stub. */
function stubClient(
...
describe("stale and post-destroy responses", () => {
    // BUG: a rename that resolves after destroy must not touch the document —
    // currently the edit is applied to the (still live) view
    it.fails(
        "does not apply rename edits after the plugin is destroyed",
        async () => {
    ...
    // BUG: edits computed against the pre-edit document must be discarded when
    // the document changed while the request was in flight — currently the
    // offsets are resolved against the current text and corrupt it
    it.fails(
        "does not apply rename edits when the document changed while the request was in flight",
        async () => {
            const client = createFakeClient();
            const view = createView("hello world");
            stubCoords(view);
            ...
            const pendingRename = deferred<LSP.WorkspaceEdit>();
            stubClient(client, {
                textDocumentRename: vi
                    .fn()
                    .mockReturnValue(pendingRename.promise),
            });
    ...
    // BUG: a definition response arriving after destroy must not move the
    // selection — currently the plugin dispatches into the live view
    it.fails(
        "does not move the selection after the plugin is destroyed",
        async () => {
            const client = createFakeClient({
    ...
    // BUG: the newest signature help response must win — currently an older
    // response that resolves late overwrites the newer tooltip
    it.fails(
        "does not show a stale signature tooltip when a newer request already resolved",
        async () => {
            const client = createFakeClient({
...
describe("request failure handling", () => {
    // BUG: a failed hover request should resolve to null — currently the
    // rejection escapes into CodeMirror's hover source
    it.fails("returns null when the hover request rejects", async () => {
        const client = createFakeClient();
        const view = createView("hello world");
        const plugin = createPlugin(view, client);
        stubClient(client, {
    ...
    // BUG: a hover request that times out should resolve to null — currently
    // the timeout rejection escapes into CodeMirror's hover source
    it.fails("returns null when the hover request times out", async () => {
        const client = createFakeClient();
        const view = createView("hello world");
        const plugin = createPlugin(view, client);
        stubClient(client, {
    ...
    // BUG: a failed completion request should resolve to null — currently the
    // rejection escapes into CodeMirror's completion source
    it.fails("returns null when the completion request rejects", async () => {
        const client = createFakeClient({
    ...
    // BUG: a CompletionList without `items` should be tolerated — currently
    // `completionList.items.map` throws a TypeError
    it.fails(
        "returns null when the server sends a CompletionList without items",
        async () => {
            const client = createFakeClient({
                capabilities: { completionProvider: {} },
            });
    ...
    // BUG: a malformed location must be ignored — currently a location without
    // a `range` throws (`posToOffsetOrZero(doc, undefined.start)`) and one
    // without a `uri` is reported as an external document
    it.fails("ignores a definition response missing uri or range", async () => {
        const client = createFakeClient({
            capabilities: { definitionProvider: true },
        });
        const view = createView("hello world");
    ...
    // BUG: a definition range that does not exist in the document should be
    // ignored — currently posToOffsetOrZero jumps the cursor to offset 0
    it.fails(
        "does not collapse the selection to offset 0 for an out-of-range definition range",
        async () => {
            const client = createFakeClient({
                capabilities: { definitionProvider: true },
            });
...
describe("document synchronization", () => {
    // BUG: didChange must never precede didOpen (and the version counter must
    // start at the version didOpen announced) — currently sendChanges only
    // checks `client.ready`, so a keystroke during initialization sends
    // didChange for a document the server has not opened yet
    it.fails(
        "does not send didChange before didOpen has completed",
        async () => {
            const order: string[] = [];
...
describe("listener and DOM leaks", () => {
    // BUG: dismissing the rename popup with Escape should remove the
    // outside-click listener — currently it is only removed from inside
    // handleOutsideClick, so every rename leaks one document listener
    it.fails(
        "removes the outside-click listener when the rename popup is dismissed with Escape",
        async () => {
            const client = createFakeClient();
            const view = createView("hello world");
            stubCoords(view);
    ...
    // BUG: destroy() should tear down an open rename popup like it does the
    // code action menu — currently the popup stays attached to document.body
    it.fails("removes an open rename popup on destroy", async () => {
        const client = createFakeClient();
        const view = createView("hello world");
        stubCoords(view);
        const plugin = createPlugin(view, client);
    ...
    // BUG: destroy() should publish an empty anchor map — currently the
    // plugin's anchors stay in the StateField and are re-mapped on every
    // subsequent document change forever
    it.fails("clears its related-location anchors on destroy", async () => {
        const view = new EditorView({
            state: EditorState.create({
                doc: "hello world",
...
describe("rename input handling", () => {
    // BUG: submitting the unchanged name should be a no-op — currently
    // `input.defaultValue` is "" (the code sets the `value` property, not the
    // attribute), so the early return is dead code and a rename is sent
    it.fails(
        "does not send a rename request when the name is unchanged",
        async () => {
            const client = createFakeClient();
            const view = createView("hello world");
            stubCoords(view);
    ...
    // BUG: an unresolvable range end must not widen the prefill — currently
    // posToOffset returns undefined and doc.sliceString(from, undefined)
    // slices to the end of the document
    it.fails(
        "does not prefill the rename input with the rest of the document",
        async () => {
            const client = createFakeClient();
            const view = createView("hello world");
            stubCoords(view);
...
describe("signature help rendering", () => {
    ...
    // BUG: a reversed [end, start] label range should not corrupt the rendered
    // signature — currently substring() swaps the bounds and the label text is
    // duplicated ("sum(aaa, b)")
    it.fails("handles a parameter label range that is reversed", () => {
    ...
    // BUG: an out-of-bounds label range should be clamped or ignored —
    // currently the whole signature is rendered as the active parameter
    it.fails("handles a parameter label range that is out of bounds", () => {
        const view = createView("sum(1, 2)");
        const plugin = createPlugin(view);
