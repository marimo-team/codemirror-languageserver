...
/**
 * Negative/protocol-conformance tests for {@link JSONRPCClient}: malformed and
 * hostile server frames, correlation edge cases, and pending-map lifetime.
 * Mirrors the ControlledTransport convention from `jsonrpc.test.ts`.
 */
class ControlledTransport implements Transport {
    ...
    constructor(private readonly autoConnect = true) {
        // The client only ever awaits this promise, but a connection that the
        // test never opens must not surface as an unhandled rejection here.
...
/** Any frame a hostile/buggy server could put on the wire. */
const frame = (value: unknown) => value as JSONRPCMessage;

/** The client's internal id→request map, for leak assertions. */
const pendingMap = (client: JSONRPCClient) =>
    (client as unknown as { pending: Map<unknown, unknown> }).pending;

/** Settle-tracking wrapper that never leaves an unhandled rejection behind. */
...
describe("malformed responses", () => {
    ...
    // BUG: a non-object `error` member should still reject with a usable
    // RPCError (a non-empty message and a numeric code) — currently
    // `response.error.code`/`.message` are `undefined` on a string/number/true,
    // producing `new RPCError(undefined, undefined)`: code `undefined` and an
    // empty message, so nothing downstream can report or classify the failure.
    it.fails(
        "rejects with a usable RPCError when the server's error member is not an object",
        async () => {
    ...
    // BUG: a response with neither `result` nor `error` is malformed and should
    // reject — currently it resolves `undefined`, which is indistinguishable
    // from a legitimate `null`/absent result and detonates later at the
    // destructuring call sites (e.g. `const { capabilities } = await ...` in
    // lsp.ts) with an opaque TypeError far from the real cause.
    it.fails(
        "rejects a response that has neither a result nor an error member",
        async () => {
            const transport = new ControlledTransport();
            const client = new JSONRPCClient(transport);
    ...
    // BUG: JSON-RPC ids are compared by value across the wire and many servers
    // echo a numeric id as a string; `"0"` should settle request `0` —
    // currently `pending` is a Map keyed by the numeric id, the string id never
    // matches, and the request hangs until its timeout fires.
    it.fails(
        "correlates a response whose id was echoed as a string",
        async () => {
            vi.useFakeTimers();
            const transport = new ControlledTransport();
            const client = new JSONRPCClient(transport);
...
describe("frame validation", () => {
    // BUG: JSON-RPC 2.0 requires every frame to declare `"jsonrpc": "2.0"`, and
    // a frame that does not should be dropped — currently the member is never
    // inspected, so any object with a `method` string is dispatched to the
    // notification/request handlers as if it were a valid LSP message.
    it.fails("ignores frames that do not declare jsonrpc 2.0", () => {
        const transport = new ControlledTransport();
        const client = new JSONRPCClient(transport);
...
describe("fire-and-forget sends", () => {
    // BUG: `notify()` is fire-and-forget at every call site (lsp.ts, plugin.ts
    // discard the promise), so a failed connection must not produce an
    // unhandled rejection — currently `dispatch()` returns the raw
    // `connected.then(...)` chain, whose rejection nothing handles, which
    // crashes strict hosts and pollutes the console for every dropped frame.
    it.fails(
        "does not produce an unhandled rejection when a notification is sent over a failed connection",
        async () => {
            const seen: unknown[] = [];
