...
describe("send after close", () => {
    it("drops frames sent after close instead of buffering them forever", async () => {
...
describe("pre-open buffering", () => {
    it("caps the pre-open outbox instead of growing without bound", () => {
        const transport = new WebSocketTransport("ws://host/lsp");
        const connected = transport.connect();
        connected.catch(() => {});
        ...
        expect(outboxOf(transport).length).toBeLessThanOrEqual(OUTBOX_LIMIT);
    });
});

describe("post-open failure", () => {
    it("surfaces a post-open socket close so in-flight requests fail fast", async () => {
        vi.useFakeTimers();
        const transport = new WebSocketTransport("ws://host/lsp");
...
describe("repeated connect", () => {
    it("does not leak or double-dispatch when connect is called twice", async () => {
        const transport = new WebSocketTransport("ws://host/lsp");
        ...
        first.emitMessage(JSON.stringify({ jsonrpc: "2.0", method: "stale" }));

        expect(first.closed).toBe(true);
        expect(handler).not.toHaveBeenCalled();
        await expect(firstConnection).rejects.toThrow(/replaced/);
    });
});
...
describe("url validation", () => {
    ...
    it("rejects a non-WebSocket URL scheme", () => {
        for (const url of [
            "http://host/lsp",
            "https://host/lsp",
            "javascript:alert(1)",
    ...
    it("warns when an insecure ws:// URL is used from a secure page", () => {
...
describe("inbound frames", () => {
    it("ignores binary frames without silently losing them", () => {
        const transport = new WebSocketTransport("ws://host/lsp");
        transport.connect().catch(() => {});
        const socket = lastSocket();
    ...
    it("keeps dispatching to remaining handlers when one throws", () => {
        const transport = new WebSocketTransport("ws://host/lsp");
        transport.connect().catch(() => {});
        const socket = lastSocket();
    ...
    it("does not dispatch inbound JSON that is not an object", () => {
        const transport = new WebSocketTransport("ws://host/lsp");
        transport.connect().catch(() => {});
        const socket = lastSocket();
